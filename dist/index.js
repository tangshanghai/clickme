...
var GameControl = function (_Event) {
    _inherits(GameControl, _Event);
    ...
    _createClass(GameControl, [{
        ...
        key: "RectItemLock",
        value: function RectItemLock(_obj) {
            //{isLock:true}
            this.dispatchEvent(this.RECTITEM_LOCK, _obj);
        }
    }, {
        key: "MoveToPos",
        value: function MoveToPos(_obj) {
            this.dispatchEvent(this.MOVE_TO, _obj);
        }
    }, {
        key: "UpdateItem",
        value: function UpdateItem(_obj) {
            this.dispatchEvent(this.UPDATE_ITEM, _obj);
        }
    }, {
        key: "CreateItem",
        value: function CreateItem(_obj) {
...
exports.default = new GameControl();
...
var Event = function () {
    ...
    _createClass(Event, [{
        ...
        key: "dispatchEvent",
        value: function dispatchEvent(type, parameter) {
            for (var i = 0; i < this.callBacks.length; i++) {
                var temp = this.callBacks[i];
                if (temp.type == type) {
...
var Configuration = function Configuration() {
	...
	this.rectWordColor = "#000000";
	//this.rectColors=['#8E8E8E','#4A4AFF','#8CEA00','#B87070','#d0d0d0','#B9B9FF','#D3FF93','#D9B3B3','#EA0000','#0080FF','#C4C400','#AFAF61','#ff7575','#97CBFF','#FFFFAA','#CDCD9A','#FF359A','#00E3E3','#D9B300','#6FB7B7','#ffaad5','#BBFFFF','#FFED97','#A3D1D1','#FF00FF','#02F78E','#FF8000','#9999CC','#ffa6ff','#ADFEDC','#FFD1A4','#C7C7E2','#9F35FF','#00EC00','#F75000','#B766AD','#d3a4ff','#A6FFA6','#FFBD9D','#D2A2CC'];
	this.rectColors = ["#be69c9", "#ffd72e", "#43d0b4", "#76d9f7", "#ff5494", "#4cb94b", "#f6613c", "#af043e", "#9d5d33", "#491864", "#be69c9", "#ffd72e", "#43d0b4", "#76d9f7", "#ff5494", "#4cb94b", "#f6613c", "#af043e", "#9d5d33", "#491864", "#be69c9", "#ffd72e", "#43d0b4", "#76d9f7", "#ff5494", "#4cb94b", "#f6613c", "#af043e", "#9d5d33", "#491864", "#be69c9", "#ffd72e", "#43d0b4", "#76d9f7", "#ff5494", "#4cb94b", "#f6613c", "#af043e", "#9d5d33", "#491864"];
	//this.rectColors = this.rectColors.reverse();
};
...
var Map = function () {
    function Map() {
        ...
        _GameControl2.default.addEventListener(_GameControl2.default.RECTITEM_CLICK, this.itemClickHandler.bind(this));
    }

    _createClass(Map, [{
        key: "itemClickHandler",
        value: function itemClickHandler(_obj) {
            var obj = _obj;
            if (obj == null) return;
            var i = obj.i,
                j = obj.j;
        ...
        key: "startMove",
        value: function startMove(arr, i, j) {
            for (var k = 0; k < arr.length; k++) {
                ...
                _GameControl2.default.MoveToPos(obj);
            }
            var self = this;
            setTimeout(function () {
                self.Maps[i][j]++;
                _GameControl2.default.UpdateItem({ i: i, j: j, v: self.Maps[i][j] });
                self.repairMaps();
            }, 1000);
        }
        ...
        value: function repairMaps() {
            for (var j = 4; j >= 0; j--) {
                ...
                for (var _i = 4; _i >= 0; _i--) {
                    if (hasArr.length > 0) {
                        var hasObj = hasArr.shift();
                        this.Maps[_i][j] = hasObj.v;
                        _GameControl2.default.MoveToPos({
                            si: hasObj.i,
                            sj: hasObj.j,
                            ei: _i,
                            ej: j,
        ...
        key: "findThree",
        value: function findThree(i, j) {
            var cv = this.Maps[i][j];
            //横向查找
            var hArr = [];
            for (var k = i - 1; k >= 0; k--) {
                if (this.Maps[k][j] == cv) {
            ...
            for (var _k = i + 1; _k < 5; _k++) {
                if (this.Maps[_k][j] == cv) {
                    hArr.push({ i: _k, j: j });
                } else {
                    break;
                }
            ...
            //竖向查找
            var vArr = [];
            for (var _k2 = j - 1; _k2 >= 0; _k2--) {
                if (this.Maps[i][_k2] == cv) {
                    vArr.push({ i: i, j: _k2 });
                } else {
                    break;
                }
            }
            for (var _k3 = j + 1; _k3 < 5; _k3++) {
                if (this.Maps[i][_k3] == cv) {
                    vArr.push({ i: i, j: _k3 });
                } else {
                    break;
                }
        ...
        value: function createMap() {
            ...
            for (var i = 0; i < 5; i++) {
                ...
                for (var j = 0; j < 5; j++) {
                    var v = Math.floor(Math.random() * this.range) + 1;
                    temparr.push(v);
                }
                mapArr.push(temparr);
            }
        ...
        key: "getNewMap",
        value: function getNewMap() {
            var mapArr = this.createMap();
            for (var i = 0; i < 5; i++) {
        ...
        key: "RemoveThree",
        value: function RemoveThree(arr) {
            for (var i = 0; i < arr.length; i++) {
                var temparr = arr[i];
                for (var j = 0; j < temparr.length; j++) {
                    var c = arr[i][j];
                    var left = j - 1 >= 0 ? arr[i][j - 1] : -1;
                    var right = j + 1 <= temparr.length - 1 ? arr[i][j + 1] : -1;
                    var top = i - 1 >= 0 ? arr[i - 1][j] : -1;
...
var RectBox = function () {
    function RectBox() {
        _classCallCheck(this, RectBox);

        this.root = (0, _jquery2.default)("<div></div>");
        var maps = _Map2.default.getNewMap();
        console.log(maps);
        for (var i = 0; i < maps.length; i++) {
            var arr = maps[i];
            for (var j = 0; j < arr.length; j++) {
                var v = arr[j];
                var ri = new _RectItem2.default({
                    value: v,
                    orderX: i,
                    orderY: j
                });
        ...
        _GameControl2.default.addEventListener(_GameControl2.default.CREATE_ITEM, this.createItemHandler.bind(this));
    }

    _createClass(RectBox, [{
        key: "createItemHandler",
        value: function createItemHandler(_obj) {
            var ri = new _RectItem2.default({
                value: _obj.v,
                orderX: _obj.i,
                orderY: _obj.j
...
var RectItem = function () {
    function RectItem(_obj) {
        _classCallCheck(this, RectItem);

        var defaultW = 60;
        var defaultH = 60;
        var wordColor = _Configuration2.default.rectWordColor || "#000000";
        ...
        this.v = _obj.value || 1;
        this.root = (0, _jquery2.default)("<div></div>");
        this.content = (0, _jquery2.default)("<div></div>");
        ...
        this.content.css({
            background: _Configuration2.default.rectColors[this.v - 1]
        });
        ...
        _GameControl2.default.addEventListener(_GameControl2.default.RECTITEM_LOCK, this.lockHandler.bind(this));
        _GameControl2.default.addEventListener(_GameControl2.default.MOVE_TO, this.moveToHandler.bind(this));
        _GameControl2.default.addEventListener(_GameControl2.default.UPDATE_ITEM, this.updateItemHandler.bind(this));
    }

    _createClass(RectItem, [{
        key: "clickHandler",
        value: function clickHandler() {
            ...
            this.v++;
            this.content.text(this.v);
            this.content.css("background", _Configuration2.default.rectColors[this.v - 1]);
            console.log("点击时", this.getOrderX(), this.getOrderY());
            _GameControl2.default.RectItemClick({
                i: this.getOrderX(),
                j: this.getOrderY()
            });
        }
        ...
        key: "lockHandler",
        value: function lockHandler(_obj) {
            this.isLock = _obj.isLock;
        }
    }, {
        key: "moveToHandler",
        value: function moveToHandler(_obj) {
            if (_obj.si == this.orderX && _obj.sj == this.orderY) {
                var left = _obj.ej * this.defaultWH;
                var top = _obj.ei * this.defaultWH;
                var self = this;
                this.root.animate({
                    left: left,
                    top: top
        ...
        value: function updateItemHandler(_obj) {
        ...
        key: "getOrderX",
        value: function getOrderX() {
            return this.orderX;
        }
    }, {
        key: "getOrderY",
        value: function getOrderY() {
            return this.orderY;
        }
    }, {
        key: "removeSelf",
...
var Explain = function Explain(_callback) {
	var _this = this;
	...
	this.root.find(".Explain_close").click(function (event) {
		console.log("关闭窗口");
	});
	this.root.find(".Explain_level1").click(function (event) {
		_this.img.attr("src", "./images/2.png");
		_this.el1.hide();
		_this.el2.show();
	});
	this.root.find(".Explain_level2").click(function (event) {
		_this.img.attr("src", "./images/3.png");
		_this.el2.hide();
		_this.el3.show();
	});
	this.root.find(".Explain_level3").click(function (event) {
		_this.img.attr("src", "./images/4.png");
		_this.el3.hide();
		_this.el4.show();
	});
	this.root.find(".Explain_level4").click(function (event) {
		console.log("开始游戏");
		_this.root.remove();
		if (_callback) {
			_callback();
...
var StartWindow = function StartWindow(_callback) {
    var _this = this;

    _classCallCheck(this, StartWindow);
...
if(false) {
	...
	if(!content.locals) {
		module.hot.accept("!!../../node_modules/css-loader/index.js!./GameWindow.css", function() {
			var newContent = require("!!../../node_modules/css-loader/index.js!./GameWindow.css");
...
if(false) {
	// When the styles change, update the <style> tags
	if(!content.locals) {
		module.hot.accept("!!../../../node_modules/css-loader/index.js!./StartWindow.css", function() {
			var newContent = require("!!../../../node_modules/css-loader/index.js!./StartWindow.css");
			if(typeof newContent === 'string') newContent = [[module.id, newContent, '']];
			update(newContent);
		});
	}
